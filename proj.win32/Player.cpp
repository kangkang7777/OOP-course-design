...
Division * Player::createDivision(Vec2 position, Vec2 velocity, int score)
{
	/*创建一个玩家分身*/
	auto division = Division::create(_playerName, _vestmentID, _keywordID, score);
...
bool Player::collideFoods(Foods * food)
{
	for (auto division : _divisionList)
	{
		if (division != NULL)
		{
			if (division->collideFoods(food))
			{
...
int Player::collideDivision(Division * division)
{
	int flag = 0;
...
Vector<Division *> & Player::getDivisionList()
{
